				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("clusterNumber") === oObject.clusterNumber;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].clusterNumber;
										} else {
											sCurrentId = aItemsNotInTheList[0].clusterNumber;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Clusters", {
											clusterNumber : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("clusterNumber"),
							title: oBindingContext.getProperty("clusterNumber")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "Clusters (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'Clusters (0)'");
							},
					...
					theListShouldBeSortedAscendingOnUnitNumber : function () {
						return this.theListShouldBeSortedAscendingOnField("users");
					},

					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("clusterNumber");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all Clusters to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "Clusters (" + iExpectedLength + ")"}),
									success : function () {
