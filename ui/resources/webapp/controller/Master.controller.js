...
sap.ui.define([
	...
	"sap/ui/Device",
	"live5/ui/model/formatter",
	"live5/ui/model/grouper",
	...
	return BaseController.extend("live5.ui.controller.Master", {
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("clusterNumber", FilterOperator.Contains, sQuery)];
			} else {
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("live5.ui.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function(oEvent) {
			...
			aFilterItems.forEach(function(oItem) {
				switch (oItem.getKey()) {
					case "Filter1":
						aFilters.push(new Filter("users", FilterOperator.LE, 100));
						break;
					case "Filter2":
						aFilters.push(new Filter("users", FilterOperator.GT, 100));
						break;
					default:
						break;
				}
		...
		_onMasterMatched: function() {
			this.getOwnerComponent().oListSelector.oWhenListLoadingIsDone.then(
				function(mParams) {
					...
					var sObjectId = mParams.firstListitem.getBindingContext().getProperty("clusterNumber");
		...
		_showDetail: function(oItem) {
			...
			this.getRouter().navTo("object", {
				objectId: oItem.getBindingContext().getProperty("clusterNumber")
			}, bReplace);
		},
