sap.ui.define([
	...
	return BaseObject.extend("live5.ui.model.GroupSortState", {
		 ...
		 * @alias live5.ui.model.GroupSortState
		 */
		...
		/**
		 * Sorts by clusterNumber, or by users
		 *
		...
		/**
		 * Groups by users, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "users") {
				...
				this._oViewModel.setProperty("/sortBy", "users");

				aSorters.push(
					new Sorter("users", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "clusterNumber");
			}
